...
6. **🏆 Firebase 커뮤니티 기능**
   ...
   - 랭킹에 SOOP·치지직 실방송 연동 상태를 함께 표시
   - 건의사항은 공개하지 않고 개발자만 Firebase Console에서 확인
7. **📡 시작 전 방송 연동**
   - 시작 화면의 `SOOP·치지직 방송 연동하기` 버튼으로 게임 시작 전에도 채널을 등록할 수 있습니다.
...
- **스테이지별 붉은 예고**: 골인 큐브에 접근하면 스테이지에 따라 0.44~0.22초 예고 스파크가 발동하며, 후반으로 갈수록 반응 시간이 짧아집니다.
...
└── src/
    ...
    │   ├── AIBully.js      # AI State Machine, 스테이지별 예고 & Overheat
